      ...
      try {
        ...
        if (purpose === "login") {
          ...
          if (userByEmail) {
            ...
            console.log(
              `[LOGIN SUCCESS] Existing user ${email} linked Google account`
            );
        ...
        if (purpose === "signup") {
          ...
          if (userByEmail) {
            return done(
              new Error("Email already in use. Please login instead.")
            );
          }
          ...
          console.log(
            `[SIGNUP SUCCESS] New user ${email} created via Google OAuth`
          );
...
OAuth.get("/account/google/login", (req, res) => {
  ...
  }&scope=profile email&redirect_uri=${encodeURIComponent(
